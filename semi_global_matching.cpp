...
OMP_PARALLEL_FOR
	for (v = 0; v < h; v++)
	{
		...
		for (int u = 0; u < w; u++)
		{
			...
			if (disp > 0 && disp < n - 1)
			{
				const int SL = _S[disp - 1];
				const int SC = _S[disp];
				const int SR = _S[disp + 1];
				const int numer = SL - SR;
				const int denom = SL - 2 * SC + SR;
				disp = disp * DISP_SCALE + (DISP_SCALE * numer + denom) / (2 * denom);
				confidence = invGamma * (std::max(SL, SR) - SC);
...
void SemiGlobalMatching::compute(const cv::Mat& I1, const cv::Mat& I2, cv::Mat& D1, cv::Mat& D2, cv::Mat& C1)
{
...
OMP_PARALLEL_FOR
	...
	C1.create(h, w, CV_32F);

	if (param_.pathType == SCAN_4PATH)
		calcDisparity<WTA4Path>(L, S, D1, D2, C1, param_.uniquenessRatio);
	else
		calcDisparity<WTA8Path>(L, S, D1, D2, C1, param_.uniquenessRatio);
